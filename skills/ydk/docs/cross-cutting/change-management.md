...
This is the foundation of trust in YDK. If something landed in main, a human approved it.
...
ydk component validate         → Layer A linker validates references
ydk spec verify --all-files     → AI evaluation passes (18 criteria including C17 density, C18 leakage)
...
```
ydk config set spec_check.thresholds.completeness 9
  → modifies .ydk/config.yaml
  → commit + push + PR
```
...
For lightweight spec modifications that don't warrant a full brainstorming cycle, YDK provides an OpenSpec-inspired delta spec system:
...
```bash
ydk change list                    # List all proposed changes
ydk change status <id>             # Check status of a specific change
ydk change diff <id>               # Show what the change modifies (before/after)
...
Human reviews and approves the change proposal
  ...
  → ydk spec verify validates the updated specs
  → PR created with applied changes
  → Human reviews and approves

ydk change archive <id>
...
| Fix a spec error | Spec evolution (`ydk change propose`) |
| Update an API contract | Spec evolution (`ydk change propose`) |
| Add a missing error scenario | Spec evolution (`ydk change propose`) |
| Change a technology choice | Full brainstorming (requires ADR + trade-off analysis) |

## Conventional Commits

All commit messages follow conventional commit format. Enforced by commit-msg hook (installed by `ydk init`).
...
Created automatically by `ydk task start`. Never created manually.

## Spec Amendments

When the spec needs to change (discovered during implementation, new requirement):

1. **Lightweight change**: Use spec evolution — `ydk change propose`, apply changes, PR
2. **Significant change**: Agent creates a new brainstorming session (Stage 01, Small Change or Major Feature mode)
   ...
   - `ydk component validate` validates references
   - `ydk spec verify` validates the amended specs
...
| Narrative spec files | Yes | `ydk spec verify` + `ydk component validate` |
| Component manifests | Yes | `ydk component validate` |
| ADRs | Yes | None (human reviews content) |
| project-rules.md | Yes | None (human reviews content) |
| .ydk/config.yaml | Yes | `ydk config validate` |
| Delta specs (docs/changes/) | Yes | Human reviews change proposal |
| Generated files | No — regenerated, not hand-edited | N/A |
| .ydk/memory/ | No — derived index, gitignored | N/A |
| .ydk/proofs/ | No — evidence files, gitignored | N/A |
| .ydk/cache/ | No — verification cache, gitignored | N/A |
