# YDK Configuration Reference

## Overview

All YDK settings live in `.ydk/config.yaml` in the project root. The config is Pydantic-validated — unknown fields are rejected, type mismatches produce clear errors.

**All config changes MUST go through the config management script.** Do NOT edit `.ydk/config.yaml` directly.
...
# ─── Components (Stage 01) ────────────────────────────
components:
  schemas_location: string        # default: ".ydk/schemas"
  components_location: string     # default: ".ydk/components"
  linker:
...
spec_check:
  ...
  results_path: string            # default: ".ydk/spec-check-results.json"
  thresholds:
...
scaffolding:
  template_location: string       # default: ".ydk/templates"
...
catalog:
  ...
  local_path: string              # default: "~/.ydk/catalog" — path to local catalog store
  chroma_path: string             # default: "~/.ydk/catalog/chroma" — ChromaDB index for semantic search

# ─── TODO Management ─────────────────────────────────
todo:
  registry_path: string           # default: ".ydk/todos.yaml" — path to TODO registry
  auto_check_on_done: bool        # default: true — check TODO resolution in `ydk task done`

# ─── Ignition ─────────────────────────────────────────
ignition:
  hash_store: string              # default: ".ydk/ignition-hashes.json" — idempotency hash tracking
  post_checks:
...
verification:
  enabled: list[string]              # default: [] — empty means all plugins run
                                     # Set by --stack during ydk init
  cache:
    enabled: bool                    # default: true — content-hash based verification caching
    location: string                 # default: ".ydk/cache/verify"
...
watch:
  ...
  sessions_file: string           # default: ".ydk/sessions.yaml" — session tracking file
```
...
All hooks enabled by default after `ydk init`. Pre-push spec/task checks disabled by default.
...
| Field | Type | Default | Description |
|---|---|---|---|
| `schemas_location` | string | `".ydk/schemas"` | Path to component schema definitions. |
| `components_location` | string | `".ydk/components"` | Path to component manifest files. |
...
| `local_path` | string | `"~/.ydk/catalog"` | Path to local catalog store. |
| `chroma_path` | string | `"~/.ydk/catalog/chroma"` | ChromaDB index for semantic search of catalog items. |

### todo

| Field | Type | Default | Description |
|---|---|---|---|
| `registry_path` | string | `".ydk/todos.yaml"` | Path to the TODO registry file, relative to project root. |
| `auto_check_on_done` | bool | `true` | Automatically verify all assigned TODOs are resolved when running `ydk task done`. |

### ignition

| Field | Type | Default | Description |
|---|---|---|---|
| `hash_store` | string | `".ydk/ignition-hashes.json"` | Path to the idempotency hash store. Tracks file content hashes to skip unchanged files on re-runs. |
...
| `cache.location` | string | `".ydk/cache/verify"` | Where to store verification cache. |
...
| Field | Type | Default | Description |
|---|---|---|---|
| `poll_interval_seconds` | int | `30` | How often the launchd plist triggers `ydk watch poll`. |
| `sessions_file` | string | `".ydk/sessions.yaml"` | Path to the session tracking file. |

## Session Tracking File

Active agent sessions are tracked in `.ydk/sessions.yaml`. The watch system reads this file to determine which Claude Code session to resume when a new review comment arrives.
...
sessions:
  T-a1b2c3d4:
    ...
    pr_number: 47
    worktree: .ydk/worktrees/T-a1b2c3d4
  ...
  T-e5f6g7h8:
    ...
    pr_number: 48
    worktree: .ydk/worktrees/T-e5f6g7h8
    status: in-progress
...
- `pr_number` — the GitHub PR number (set after `ydk task done`)
...
The watch system uses a lock file (`.ydk/watch.lock`) to prevent concurrent polls. If a poll is already in progress, subsequent invocations exit immediately.
