...
`ydk memory audit` flags expired research entries.

**Discovery:** Before researching externally, agents SHOULD check `docs/research/` and `ydk memory search` for existing research on the topic.
...
### Stage 04 (Learning)
- `ydk memory audit` checks research cache freshness
