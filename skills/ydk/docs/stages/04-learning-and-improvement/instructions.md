...
**Constraints:**
- You MUST run `ydk memory index` after adding or modifying any knowledge file
- Indexing is idempotent — running it multiple times is safe
- Index covers: `docs/specs/`, `.ydk/components/`, `docs/adrs/`, `docs/research/`, `docs/project-rules.md`
- Large files are chunked automatically; sections become individually searchable
- Component manifests are indexed with their `ydk:` IDs as metadata, enabling search by component reference
...
```bash
ydk memory extract T-a1b2c3d4 --jsonl /path/to/session.jsonl
...
**Constraints:**
- You SHOULD record decisions that affect future tasks using `ydk memory record-decision TOPIC`
...
```bash
ydk memory record-decision "order-validation-strategy" \
...
**Constraints:**
- You SHOULD run `ydk memory consolidate` after sprints with many extractions
...
**Constraints:**
- You SHOULD run `ydk memory audit` at the start of each sprint
...
- New templates go in `.ydk/templates/`
- Template creation is a task itself — create a discovery task with `ydk task add-subtask`
...
Procedural effectiveness analysis is now included as part of `ydk memory audit`. The audit command provides a comprehensive health check including stale research, duplicates, and procedural reports.

**Constraints:**
- You SHOULD run `ydk memory audit` after each sprint
...
ChromaDB is an optional dependency. Install it with: `uv pip install 'ydk[memory]'`
...
The quality of bootstrap depends on the quality of the index and the memory scores. Run `ydk memory index` to ensure the index is current, check that spec refs and component_refs on the task are accurate, and run `ydk memory consolidate` to reduce noise from duplicates.
...
Run `ydk memory consolidate` to merge duplicates, then `ydk memory audit` to identify candidates for archiving.
