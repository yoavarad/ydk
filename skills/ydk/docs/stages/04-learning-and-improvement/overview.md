...
This means `ydk memory search "rate limiting"` finds content about throttling AND content that literally mentions "rate limiting" — the best of both worlds.
...
### Decision Memory
`ydk memory record-decision TOPIC` records topic-keyed decisions in an append-only log. Latest entry wins. When searching for "how do we handle X?", the most recent decision is returned.
...
### Consolidation
`ydk memory consolidate` merges duplicate or near-duplicate memories into single authoritative entries. Reduces noise and context window waste.

### Procedural Memory
`ydk memory audit` includes procedural effectiveness analysis — tracking which prompts, approaches, and patterns were effective across tasks.
...
```
ydk memory index              → Index all project knowledge files into ChromaDB
ydk memory search <query>     → Hybrid search (vector + BM25 + RRF) with scoring
ydk memory extract T-xxx      → Extract learnings (discoveries, decisions, gotchas, abandoned)
ydk memory record-decision TOPIC → Record topic-keyed decision (append-only, latest-wins)
ydk memory retrospective      → Aggregate learnings across a sprint
...
ydk memory bootstrap T-xxx    → Assemble context for a new task automatically
```
