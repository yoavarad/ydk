...
**Storage:** ChromaDB, a lightweight vector database that persists to disk at `.ydk/memory/chroma/`. No external services required for storage.
...
**Keyword Index:** BM25 index maintained alongside the vector store for keyword-based matching. Updated automatically during `ydk memory index`.
...
`ydk memory index` reads all knowledge files from configured locations:

- `docs/specs/` — narrative specification documents
- `.ydk/components/` — component manifests (indexed with `ydk:` IDs as metadata)
- `docs/adrs/` — architectural decision records
- `docs/research/` — cached research on external technologies
- `docs/project-rules.md` — project conventions and gotchas

Each file is split into chunks (by markdown sections or YAML documents where possible). Each chunk gets an embedding and is stored with metadata: source file, section heading, file type, last modified date, `ydk:` ID (for manifests).
...
`ydk memory search <query>` runs three search strategies and merges results:
...
```bash
ydk memory search "rate limiting"                     # Default: hybrid (vector + BM25 + RRF)
ydk memory search "rate limiting" --mode vector       # Vector only
ydk memory search "rate limiting" --mode keyword      # BM25 only
ydk memory search "rate limiting" --mode hybrid       # Explicit hybrid
```
...
```bash
ydk memory search "rate limiting" --depth index       # IDs, titles, scores only
ydk memory search "rate limiting" --depth summary     # Key points and summaries
ydk memory search "rate limiting" --depth full        # Complete content
```
...
ydk memory get mem-001 mem-042
...
Expired memories are NOT deleted — they're retained for audit but filtered from active search. `ydk memory audit` flags expired entries.
...
`ydk memory bootstrap <task-id>` is a specialized search that assembles context for a specific task. It combines:
...
The result is a focused context package that gives the agent a head start. When `memory.auto_bootstrap: true`, this runs automatically during `ydk task start`.

## Configuration

All memory settings live in `.ydk/config.yaml` under the `memory` key:
...
- Search quality depends on index freshness. Run `ydk memory index` after modifying knowledge files.
