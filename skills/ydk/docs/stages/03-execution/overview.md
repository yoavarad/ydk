...
```
ydk task start <id>      → Claim + explore + read component_refs + post understanding to issue
ydk task comment <id>       → Post implementation plan to issue
                         → Agent writes code (TDD: red → green → refactor)
ydk task comment <id>   → Post progress updates throughout
...
- Branch and worktree created automatically by `ydk task start`
...
- Commit message format validated by commit-msg hook (installed by `ydk init`)
...
### Auto-Fix
`ydk verify run --auto-fix` runs `ruff --fix` before checking, automatically resolving common lint issues.

### Auto-Repair Loop
`ydk verify run --retry N --repair` retries failed verifications up to N times, applying structured fixes between retries. The repair system parses verification error output and attempts targeted fixes.

### Verification Caching
Verification results are cached by content hash. If code hasn't changed since last verification, results are reused. Bypass with `--no-cache`. Clear cache with `ydk verify clear-cache`.

### PR Commenting
`ydk verify run --pr <URL>` posts verification results as a comment on the specified PR. Useful for CI integration and visibility.

### Commit-Message Hook
`ydk init` installs a commit-msg hook that validates conventional commit format. Rejects commits that don't match `<type>(<scope>): <description>`.

### TDD Phase Tracking
`ydk task tdd <id> --stage red|green|refactor` records the current TDD phase on a task for observability.

### Test Generation
`ydk test generate --from <COMPONENT_ID>` generates test stubs from a component manifest. `ydk test coverage` shows test coverage for all components.
