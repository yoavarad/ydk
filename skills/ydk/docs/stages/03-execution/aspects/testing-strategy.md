...
Every project using YDK has three test levels with strict structure.
...
2. Run → 11 FAIL (ImportError)
   → ydk task comment T-002 "RED: 11 tests written, all failing"

3. Implement first function — enough for 5 tests
4. Run → 5 PASS, 6 FAIL
   → ydk task comment T-002 "GREEN partial: 5/11 passing"

5. Implement second function — enough for remaining
6. Run → 11 PASS
   → ydk task comment T-002 "GREEN: 11/11 passing"
...
| Trigger | Tests |
|---|---|
| During TDD development | `ydk verify tests --unit` (fast feedback) |
| Pre-push hook | All levels (unit + integration + E2E) |
| `ydk verify tests` | All levels |
| `ydk verify tests --unit` | Unit only |
| `ydk verify tests --integration` | Integration only |
| `ydk verify tests --e2e` | E2E only |
