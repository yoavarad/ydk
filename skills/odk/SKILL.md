...
- **Component manifests** — structured YAML files in `.odk/components/`. Machine-readable definitions of every entity, route, error, contract, requirement, NFR, and external dependency.
...
- **Layer A (deterministic linker)** — validates that every `[odk:...]` reference in narratives points to an existing component manifest. Fast, runs as part of `odk verify`.
...
```bash
odk component list                    # List all component manifests
odk component show <id>               # Show a specific manifest
odk component create <type> <id>      # Create a new manifest from schema
odk component validate                # Validate all manifests against schemas
odk component list-schemas            # List available schemas
odk component show-schema <type>      # Show a schema definition
odk component init-schemas            # Initialize .odk/schemas/ with defaults
```
...
```bash
odk init --name <project>              # Initialize .odk/ config, install hooks (pre-commit, commit-msg, pre-push), install guard hooks in .claude/settings.json
odk config show|get|set|validate       # Config management
```
...
```bash
odk catalog search <query>                 # Semantic search for ignition packs and components
odk catalog install <name>                 # Install a catalog item into the project
odk catalog list                           # List installed catalog items
odk catalog info <name>                    # Show details of a catalog item
odk catalog publish <path>                 # Publish a local item to the catalog (validates: skeleton compiles, imports resolve, tests collect, app starts)
...
```bash
odk todo list                              # List all registered TODOs
odk todo show <id>                         # Show details of a specific TODO (ODK-TODO-NNN)
odk todo assign <todo-id> <task-id>        # Link a TODO to a task
odk todo auto-assign [--apply]             # Match TODOs to tasks by file path
odk todo assign-batch <mapping>            # Bulk assign from YAML or inline
odk todo done <todo-id>                    # Mark a TODO as resolved
odk todo coverage                          # Show TODO resolution coverage
```
...
```bash
odk ignite                                 # Generate runnable skeleton from installed ignition pack + components
odk ignite --dry-run                       # Preview what would be generated without writing files
odk ignite --force                         # Regenerate even if hashes match (override idempotency)
odk ignite --skip-verify                   # Bypass spec verification gate (for inherited/brownfield specs)
```

**What ignition does:**
1. Reads installed ignition pack + ODK component manifests
...
- Later phases consume artifacts from earlier phases via `ODK_ARTIFACT_*` env vars
...
```bash
odk spec verify                        # Run all 10 YAML-based reviewers (N01-N10) in parallel
odk spec verify --all-files            # Check all spec files (not just git-changed)
odk spec verify --verbose              # Show per-reviewer timing, cache metrics, and DEBUG logs
odk spec list-criteria                 # List reviewers and their thresholds
```
...
```bash
odk task create-epic                   # Create a new epic
odk task create-story                  # Create a new story under an epic (supports --component-refs, --spec-refs, --acceptance)
odk task create                        # Create a task under a story (supports --component-refs, --spec-refs, --acceptance, --dry-run)
odk task create-batch --from batch.yaml  # Batch create epics/stories/tasks from YAML with two-pass dep resolution
odk task create-batch --from X --dry-run # Validate batch YAML without creating issues
odk task validate-dag                  # DAG validation — shows "Critical path (dependency-only)"
odk task plan-waves --agents N         # Resource-constrained schedule — shows "Critical chain (resource-constrained)"
odk task coverage [--spec-dir X]       # Spec-to-story coverage check (respects coverage_exclude config)
odk task component-coverage            # Check every component manifest is referenced by a task
odk task component-coverage --exclude 'odk:page:*' --strict  # With exclusion patterns and strict exit code
odk task analyze-complexity            # LLM scores tasks 1-10, flags tasks needing splits
odk task add-gate <id> --type <type>   # Add external gate (pr-merged, ci-passed, timer, human)
odk task ready                         # List all actionable tasks ranked by priority
odk task list --epic E --story S --status open  # Filter tasks by epic, story, status; grouped by status
odk task archive-done [--all-done]     # Archive completed tasks for context efficiency
odk task scaffold-batch                # Generate batch YAML from existing TODOs
```
...
- Installed by `odk init` alongside `permissions.allow` in `.claude/settings.json` for non-interactive use
...
**SubagentStop hook:**
- Blocks session end if `.odk/active-task.json` exists
- Agent must run `odk task done` before session can end
- Installed by `odk init`
...
- Tracks state in `.odk/tdd-state.json`
...
**Stage Gate details:**
- Stage tracked in `.odk/state.json`
...
```bash
odk task start <id>                    # Claim task, create worktree (branch names sanitized: colons/slashes/dots stripped), explore
odk task start <id> --base <branch>    # Branch from specified base (default: HEAD, not always main)
odk task start <id> --force            # Restart a stale task (re-create worktree)
odk task comment <id> "msg"            # Post comment to task issue (replaces plan and progress)
odk task block <id> --reason X         # Mark task as blocked
odk task done <id>                     # Verify + create PR + post proof (prints pass/fail per plugin, shows PR URL or first 3 lines of failure)
odk task done <id> --skip-plugin <name>  # Skip a genuinely failing plugin (verified internally)
odk task add-subtask <id>              # Create discovered subtask linked to parent
odk task tdd <id> --stage red|green|refactor  # Set TDD phase on task
odk task quick "description"           # Fast path for small changes
odk scaffold list|info|apply|create    # Template scaffolding
odk verify run --trigger pre-commit    # Run pre-commit verifications
odk verify run --trigger pre-push      # Run all pre-push verifications
odk verify run --auto-fix              # Run ruff --fix before checking
odk verify run --retry N               # Auto-repair loop: retry N times with structured error output
odk verify run --repair                # Auto-repair: fix issues automatically between retries
odk verify run --pr <URL>              # Post verification results as PR comment
odk verify run --no-cache              # Bypass verification cache
odk verify list                        # List available plugins
odk verify clear-cache                 # Clear the verification content-hash cache
odk test generate --from <COMPONENT_ID>  # Generate tests from a component manifest
odk test coverage                      # Show test coverage report for all components
```
...
```bash
odk memory index                       # Index project knowledge into ChromaDB
odk memory search <query>              # Semantic search
odk memory search <query> --mode hybrid  # Vector + BM25 keyword + RRF fusion (default)
odk memory search <query> --depth index|summary|full  # Progressive retrieval depth
odk memory bootstrap <task-id>         # Assemble context for a task
odk memory extract <task-id>           # Extract learnings from session transcript
odk memory retrospective --sprint X    # Sprint-level aggregation
...
odk memory decision-history <TOPIC>    # Show all versions of a decision for a topic
...
```bash
odk change propose                     # Create a delta spec in docs/changes/
odk change list                        # List proposed changes
odk change status <id>                 # Check change status
odk change archive <id>                # Move completed change to docs/changes/archive/
odk change diff <id>                   # Show what the change modifies
```

### Watch System

```bash
odk watch install                      # Install macOS launchd plist, polls every 30 seconds
odk watch poll                         # Check GitHub PRs for new review comments, react 👀, trigger Claude Code session
...
```bash
odk task review-comments <id>          # Fetch PR review comments for a task
```

### Visual Companion

```bash
odk visual start                       # Start browser-based mockup server
odk visual stop                        # Stop the server
odk visual push                        # Push current mockup state
odk visual feedback                    # Collect feedback on mockups
odk visual screenshot                  # Capture mockup screenshot
odk visual list                        # List all mockup sessions
```
...
1. **New project**: Enter Stage 01 → `odk catalog search` for packs → brainstorm specs + produce component manifests → run spec-check → PR
2. **Specs ready**: Enter Stage 01.5 → `odk ignite` → verify skeleton runs + app starts → review TODOs
3. **Skeleton ready**: Enter Stage 02 → decompose with component_refs → `odk todo assign` TODOs to tasks → validate DAG → plan sprint
4. **Task assigned**: Enter Stage 03 → implement TODOs with TDD (guards auto-enforced) → `odk task done` shows pass/fail per plugin → PR → review
5. **Sprint done**: Enter Stage 04 → capture lessons → update ADRs/rules
6. **Quick fix**: `odk task quick "description"` → fast path for small changes
7. **Not sure**: `odk doctor` → check environment health and get next steps
