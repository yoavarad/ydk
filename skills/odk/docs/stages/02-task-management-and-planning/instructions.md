...
- `component_refs` validated against `.odk/components/`
...
> Spec refs: orders.md
> Component refs: odk:entity:orders/Order, odk:route:orders/create
>
> Acceptance criteria:
...
**Constraints:**
- You MUST run the coverage check: `odk task coverage <spec_location>`
- You MUST NOT proceed if coverage check fails
- You SHOULD scope the check to changed spec files for amendments
- You MAY configure `task_management.coverage_exclude` in `.odk/config.yaml` to skip reference docs (glossary, scope, etc.)
- You SHOULD run `odk task component-coverage` to verify every component manifest is referenced by a task
- You MAY use `odk task component-coverage --exclude 'odk:page:*' --strict` to exclude patterns and enforce exit code

**Example:**

> ```bash
> odk task coverage docs/specs/
> # Coverage check PASSED — all spec sections have stories.
>
> odk task component-coverage --strict
> # All 24 components covered by tasks.
>
> # With exclusions for reference-only components:
> odk task component-coverage --exclude 'odk:page:*' --exclude 'odk:glossary:*' --strict
> ```
...
> Agent reads: orders.md, overview.md#cross-cutting, overview.md#testing-strategy, plus component manifests `odk:entity:orders/Order`, `odk:route:orders/create`, `odk:error:orders/insufficient-balance`
...
After ignition (Stage 01.5), every `NotImplementedError` placeholder is registered as a TODO with an `ODK-TODO-NNN` ID. Link these TODOs to tasks so that `odk task done` can verify resolution.

**Constraints:**
- You MUST run `odk todo list` to see all registered TODOs
- You MUST use `odk todo auto-assign --apply` as the PRIMARY method — it matches TODOs to tasks by file path and component_refs automatically
- For any TODOs not auto-assigned, use `odk todo assign-batch <mapping>` for bulk assignment from YAML or inline mapping
- Use individual `odk todo assign <todo-id> <task-id>` only for one-off corrections
...
- You SHOULD run `odk todo coverage` to verify all TODOs are assigned before proceeding
- A single task MAY have multiple TODOs assigned to it if they are closely related
- You MAY use `odk task scaffold-batch` to generate a batch YAML from existing TODOs as a starting point

**Example:**

> ```bash
> odk todo list
> # ODK-TODO-001  src/domain/services/order_service.py:23     place_order() business logic
> # ODK-TODO-002  src/domain/services/order_service.py:45     cancel_order() business logic
> # ODK-TODO-003  src/domain/validation/order_validator.py:12  validate_place_order() rules
>
...
> odk todo assign-batch "ODK-TODO-021:T-m3n4o5p6,ODK-TODO-022:T-e5f6g7h8"
>
> # ONE-OFF: individual corrections
> odk todo assign ODK-TODO-023 T-m3n4o5p6
>
> odk todo coverage
...
- `--component-refs` values are validated against real files in `.odk/components/` at creation time
...
> Component refs: [odk:entity:orders/Order]
> Spec refs: [orders.md#entities, overview.md#cross-cutting]
>
> Description:
> Create Order SQLAlchemy model matching the odk:entity:orders/Order manifest:
...
- Tasks implementing entities → scaffold from `odk:entity:*` component
- Tasks implementing routes → scaffold from `odk:route:*` component
- Tasks implementing contracts → scaffold from `odk:contract:*` component

Add scaffold info to task description:
"Scaffold: `odk scaffold apply fastapi-route --from odk:route:orders/create`"
...
- You MUST run DAG validation: `odk task validate-dag`
...
- You SHOULD run `odk task plan-waves --agents N` for the resource-constrained critical chain
- You MAY add gates for external blocking conditions: `odk task add-gate <id> --type pr-merged|ci-passed|timer|human`
...
- You MUST run `odk task analyze-complexity` to score all tasks 1-10
...
- You SHOULD run `odk task plan-waves --agents N` to get a resource-constrained schedule
...
Agent-driven continuous loop. Use `odk task ready` to find actionable tasks.

**Constraints:**
- You MUST query for unblocked tasks using `odk task ready` (ranks by priority, respects gates)
- You MUST claim tasks before working: `odk task start <id>`
...
Track through GitHub/GitLab native tools + ODK sprint health.
...
- You SHOULD run `odk task archive-done --all-done` to compress completed tasks for context efficiency
- For urgent work: You MUST surface impact ("Sprint has N days left, recommend deferring X")
- For requirement changes: You MUST go through Stage 01 brainstorming → spec amendment PR (or use `odk change propose` for lightweight changes)
...
Check if the task is too large (`odk task analyze-complexity` — split it), the spec is ambiguous (amend it), or the agent needs a decision (label `blocked-by-decision`).
...
### Task IDs collide
This should not happen with hash-based IDs. If it does, regenerate the colliding task with `odk task create` — the new hash will be different.
