...
- An ignition pack installed via `odk catalog install <pack-name>` (checked during Step 0 of Stage 01)
...
**Constraints:**
- You MUST run `odk ignite --dry-run` before running ignition for real
...
> #   src/domain/models/order.py          (from odk:entity:orders/Order)
> #   src/domain/models/user.py           (from odk:entity:auth/User)
> #   src/ports/order_repository.py       (from odk:contract:orders/order-repo)
> #   src/adapters/api/routes/orders.py   (from odk:route:orders/create, odk:route:orders/list)
...
**Constraints:**
- You MUST run `odk ignite` (without `--dry-run`) to generate the skeleton
...
- You MAY use `odk ignite --force` to regenerate all files, overriding idempotency

**What ignition does:**
1. Reads the installed ignition pack and all component manifests in `.odk/components/`
...
4. Registers a `ODK-TODO-NNN` for every `NotImplementedError` placeholder
...
- Phases can export artifacts (e.g., `openapi.json`) consumed by later phases via `ODK_ARTIFACT_*` env vars
...
> # Ignition pack: hexagonal-architecture
> # Generated: 47 files
> # Registered: 23 TODOs (ODK-TODO-001 through ODK-TODO-023)
> # Post-checks:
...
- You SHOULD run `odk todo coverage` to see overall resolution status (should be 0% at this point)
...
> # ODK-TODO-004  src/adapters/repos/order_repository.py:18   create() implementation
> # ...
...
Run `odk catalog list` to check installed packs. Install one with `odk catalog install <name>`.
...
The ignition pack may have assumptions about project structure that do not match your setup. Check `odk catalog info <pack-name>` for expected directory layout. The post-generation circular import check should catch this.

### Want to regenerate after component changes
Run `odk ignite` again — hash tracking means only changed files are regenerated. Use `odk ignite --force` to regenerate everything.
