...
- **Component manifests** — structured YAML files in `.odk/components/` defining every entity, route, error, contract, requirement, NFR, and external dependency
...
- Component manifests exist in `.odk/components/` for all entities, routes, errors, contracts, requirements, NFRs, and external dependencies
- Every `[odk:...]` reference in narratives resolves (Layer A linker passes)
- Layer B scanner finds no unlinked concepts in prose
- Spec quality check passes all rubrics (run `odk spec verify` — now 18 criteria including C17 density and C18 implementation leakage)
