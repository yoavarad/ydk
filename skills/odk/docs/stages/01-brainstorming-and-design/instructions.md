...
- **existing_docs** (optional): Path to existing documentation that defines the system design. When present in Rewrite mode, these docs ARE the design — you translate them to ODK format rather than brainstorming from scratch.
...
**Constraints:**
- You MUST run `odk catalog search <architecture-keywords>` to find relevant ignition packs (e.g., `odk catalog search "fastapi hexagonal"`)
- You MUST run `odk catalog list` to see what is already installed
- You SHOULD install an ignition pack before finalizing component manifests: `odk catalog install <pack-name>`
- You MUST run `odk catalog info <pack-name>` to understand what component types the pack consumes and what code it generates
...
> odk catalog info hexagonal-architecture
...
- **Rewrite mode**: The system already exists with complete documentation. The design is FIXED — you translate existing docs into ODK specs (narratives + component manifests) rather than brainstorming from scratch. Skip Steps 2-13 (questions/research) and go directly to Step 14 (narratives) using existing_docs as your complete input.
...
Define every entity with ALL fields, types, constraints, relationships, enums, state transitions. No placeholder language. **Every entity MUST be created as a component manifest in `.odk/components/`.**
...
- You MUST create a component manifest for each entity: `odk component create entity <namespace>/<name>`
- You MUST reference entities in narratives using `[odk:entity:<namespace>/<name>]`

**Example — Order entity manifest:**

> ```yaml
> # .odk/components/entities/orders/Order.yaml
> $schema: ../../schemas/entity.yaml
> id: odk:entity:orders/Order
> name: Order
> fields:
>   - name: id
>     type: UUID
...
- You MUST create a component manifest: `odk component create external-dep <namespace>/<name>`

**Example:**

> ```yaml
> # .odk/components/external-deps/exchange/binance-rest.yaml
> $schema: ../../schemas/external-dep.yaml
> id: odk:external-dep:exchange/binance-rest
...
- You MUST create route manifests: `odk component create route <namespace>/<name>`
- You MUST create error manifests for non-trivial error scenarios: `odk component create error <namespace>/<name>`

**Example:**

> ```yaml
> # .odk/components/routes/orders/create.yaml
...
- You MUST create error manifests: `odk component create error <namespace>/<name>`
...
- You MUST create NFR manifests: `odk component create nfr <namespace>/<name>`
...
- You MAY use `odk visual start` to launch the browser-based mockup tool for interactive design
- You MAY use `odk visual push` to share mockups and `odk visual feedback` to collect human input
- You MAY use `odk visual screenshot` to capture mockup screenshots for the spec
...
6. **Component reference check** — Verify every entity, route, error, NFR, and external dependency mentioned in narratives has a corresponding component manifest. Verify every `[odk:...]` link resolves. Fix any missing manifests or broken links.
...
- **ALWAYS reference components by `[odk:type:namespace/name]` ID** — every entity, route, error, NFR, and external dependency mentioned in prose must have an inline link
...
- **The component must exist in `.odk/components/`** for the reference to resolve — create it first with `odk component create`
- **Run `odk component validate` frequently** during writing to catch broken references early
...
- You MUST ensure all component manifests exist in `.odk/components/` with valid `$schema` references
- You MUST run the Layer A deterministic linker to validate all `[odk:...]` references: `odk component validate`
- You MUST run the Layer B LLM scanner to find unlinked concepts (runs as part of `odk spec verify`)
- You MUST run the spec quality check: `odk spec verify` (10 YAML-based reviewers — N01-N10, covering problem statement, success criteria, scope, terminology, ambiguity, flow completeness, information density, no-tech-in-prose, component references, and YAGNI)
...
> .odk/components/entities/orders/Order.yaml, entities/auth/User.yaml, ...
> .odk/components/routes/orders/create.yaml, routes/orders/list.yaml, ...
> .odk/components/errors/orders/insufficient-balance.yaml, ...
> .odk/components/external-deps/exchange/binance-rest.yaml, ...
> .odk/components/nfrs/system/latency-p95.yaml, ...
...
ODK ships with 13 default schemas (entity, route, error, contract, etc.) but projects can create custom schemas when the defaults don't fit. If your project has domain-specific component types that don't map well to the built-in types, create a custom schema in `.odk/schemas/`.
...
**How to create one:**
1. Create a YAML file in `.odk/schemas/<type-name>.yaml` following the schema format
2. Define `name`, `description`, `version`, and `fields` with types, requirements, and descriptions
3. Create components using `$schema: "odk:schema:<type-name>"` and `id: "odk:<type-name>:<namespace>/<name>"`

**Example — ODK's own `cli-command` schema:**

ODK itself uses `cli-command`, `core-module`, and `pydantic-model` custom schemas because its CLI commands don't fit the `route` schema (which expects HTTP methods), its core modules don't fit `contract` (which expects service interfaces), and its Pydantic models don't fit `entity` (which expects database models).
...
```
odk memory extract <task-id> --jsonl <path-to-session-jsonl>
```

## Troubleshooting

### User wants to skip brainstorming
Explain: every hour spent on specs saves 10 hours of implementation rework. Even simple changes benefit from a 5-minute brainstorm (Small Change mode). For truly trivial changes, suggest `odk task quick "description"` instead.
...
- **N08 (No Technical Specs in Prose)** — type annotations or JSON shapes found in narrative by `scan_type_annotations`. Move them to component manifests and replace with `[odk:...]` references.
- **N09 (Component References)** — unlinked entity/route mentions found by `scan_unlinked_mentions`. Add `[odk:...]` links or create missing manifests.
...
An `[odk:...]` reference in a narrative points to a component manifest that doesn't exist. Create the missing manifest with `odk component create` or fix the reference.

### Layer B scanner finds unlinked concepts
The scanner identified a concept in prose that should reference a component manifest. Add the `[odk:...]` link or create the manifest if it doesn't exist.
