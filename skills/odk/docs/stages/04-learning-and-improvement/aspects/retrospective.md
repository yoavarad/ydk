...
`odk memory audit` generates an effectiveness analysis across all sprint tasks:
...
- **Checkpoint preview value** — how many issues were caught by checkpoint preview that would have failed at `odk task done`
...
```bash
odk memory search "blocked" --mode hybrid
odk memory search "retry" --mode hybrid
odk memory search "workaround" --mode hybrid
odk memory search "abandoned" --mode hybrid
```
...
- Record key decisions via `odk memory record-decision TOPIC`
...
```bash
odk memory consolidate    # Merge duplicate memories from the sprint
odk memory index          # Re-index all project knowledge files
```
...
- The retrospective is a process, not just a command. `odk memory retrospective` provides the data, but the analysis and follow-up actions require judgment.
...
- Procedural memory analysis (`odk memory audit` with procedural analysis) is the key new capability — it tells you WHAT WORKS, not just what happened.
