...
Every ODK command that changes task state automatically posts an update to the task issue. These updates are asynchronous — you don't wait for them. You call the command and continue working.

The task issue becomes a timeline of exactly what happened:

```
10:00 — odk task start T-002
         "Exploration complete. Loaded orders.md#entities, found pattern
          at user_validator.py. 2 files to create, 0 to modify."

10:02 — odk task comment T-002 "PLAN: 11 tests planned. TDD: write tests → implement validator →
          implement cancel logic. Estimated: 2 files, ~230 lines."

10:05 — odk task comment T-002 "RED: 11 tests written, all failing"

10:12 — odk task comment T-002 "GREEN: 7/11 passing"

10:18 — odk task comment T-002 "GREEN: 11/11 passing"

10:19 — odk task done T-002
...
- Running `odk task list` to see sprint status
...
```bash
odk task list                 # List tasks in current sprint
odk task ready                # List actionable tasks ranked by priority
odk doctor                    # Check ODK environment health
```

## When Updates Are Posted

ODK posts to the task issue at these milestones — no more, no less:

| ODK Command | What gets posted |
|---|---|
| `odk task start` | Exploration summary (spec refs loaded, files identified, dependencies verified) |
| `odk task comment` | Implementation plan (files, tests, TDD sequence) |
| `odk task comment` | Progress message (you provide the message) |
| `odk task block` | Blocked reason + what's needed from human |
| `odk task done` | Verification proof summary + PR link |
| `odk task add-subtask` | New task created, linked to origin |

Labels are also updated automatically:
- `odk task start` → adds `in-progress`
- `odk task block` → adds `blocked-by-code` or `blocked-by-decision`, removes `in-progress`
- `odk task done` → adds `in-review`, removes `in-progress`
...
**Hands-off mode:** Check `odk task list` whenever you want. If no tasks are blocked, everything is proceeding.
...
- `odk task list` and GitHub/GitLab issue labels show these
