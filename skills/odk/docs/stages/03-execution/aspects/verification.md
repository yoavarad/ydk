...
Verifications are **plugins**, not hardcoded ODK features. ODK provides the runner and interface. The actual checks are external and extensible — adding a new verification means creating a folder, not modifying ODK code.
...
ODK searches project-specific first, then global. Project plugins can override global ones with the same name.
...
requires: ["ruff"]          # Tools that must be installed (checked by odk doctor)
...
| `manual` | `odk verify run` command | As specified by `--trigger` flag |

### Spec Verification (separate from verification plugins)

The `odk spec verify` command runs a **separate** system from verification plugins. It uses 10 YAML-based reviewers (N01-N10) that evaluate narrative spec quality via the Bedrock Converse API. See `stages/01-brainstorming-and-design/aspects/enforcement-gate.md` for details.

The verification plugin system (`odk verify run`) handles code-level checks (lint, types, tests, architecture). The spec reviewer system (`odk spec verify`) handles spec-level quality. They are complementary but independent.
...
```bash
odk verify run                          # Run all for current context
odk verify run --trigger pre-commit                # Only fast deterministic checks
odk verify run --trigger pre-push                # Layers 1 + 2
odk verify run --trigger pre-push                  # All layers (1 + 2 + 3)
odk verify run --name lint-ruff         # Run specific plugin
odk verify run --trigger pre-commit     # Run all pre-commit plugins
odk verify run --trigger pre-push       # Run all pre-push plugins
odk verify run --auto-fix               # Run ruff --fix before checking
odk verify run --retry N                # Retry failed checks up to N times
odk verify run --repair                 # Auto-repair: fix issues between retries
odk verify run --retry 3 --repair       # Combined: retry 3 times with auto-repair
odk verify run --pr <URL>               # Post results as PR comment
odk verify run --no-cache               # Bypass verification cache
odk verify list                         # List all available plugins
odk verify all --save-proof             # Run all + save proof artifacts
odk verify all --task-id T-a1b2c3d4     # Associate proof with a task
odk verify clear-cache                  # Clear the verification content-hash cache
```

## Auto-Fix

`odk verify run --auto-fix` runs `ruff --fix` and `ruff format` before evaluating lint checks. This automatically resolves:
...
`odk verify run --retry N --repair` provides an automated fix-and-retry cycle:
...
- **Cache location**: `.odk/cache/verify/`
...
**When caching helps:**
- Running `odk verify run` multiple times during development when only some files changed
- Re-running after fixing one issue — unchanged checks are instant

**When to bypass:**
- `--no-cache` flag forces all checks to run fresh
- `odk verify clear-cache` purges the entire cache
- Cache is automatically invalidated when plugin manifests change

## PR Commenting

`odk verify run --pr <URL>` posts verification results as a comment on the specified PR:

```bash
odk verify run --trigger pre-push --pr https://github.com/org/repo/pull/47
...
Hooks installed by `odk init` in `.odk/hooks/`:
...
Configured via `.odk/config.yaml`:
```yaml
hooks:
  pre_commit:
...
```bash
odk verify create no-float-financials --trigger git:pre-push
```

Creates `.odk/verifications/no-float-financials/` with manifest.yaml template and empty check.py.
...
Every `odk verify run` produces a VerificationReport saved to `.odk/proofs/`:
