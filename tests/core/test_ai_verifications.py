...
class TestSpecAlignmentSkips:
    ...
    def test_skip_when_aws_credentials_missing(self, tmp_path: Path) -> None:
        mod = _load_check_module(SPEC_ALIGNMENT_PATH, "spec_alignment_check")
        ...
        # Mock strands as available but boto3 STS call fails
        mock_strands = MagicMock()
        mock_bedrock = MagicMock()
        mock_boto3 = MagicMock()
        mock_boto3.Session.return_value.client.return_value.get_caller_identity.side_effect = Exception(
            "No credentials"
        )
        ...
        with patch.dict(
            sys.modules,
            {
                ...
                "strands.models.bedrock": mock_bedrock,
                "boto3": mock_boto3,
            },
        ):
            result = mod.run_check(ctx)
...
class TestSpecAlignmentWithMockedAgent:
    ...
    def test_passing_evaluation(self, tmp_path: Path) -> None:
        mod = _load_check_module(SPEC_ALIGNMENT_PATH, "spec_alignment_check")
    ...
    def test_failing_evaluation(self, tmp_path: Path) -> None:
        mod = _load_check_module(SPEC_ALIGNMENT_PATH, "spec_alignment_check")
...
class TestAiCodeReviewWithMockedAgent:
    ...
    def test_clean_review_passes(self, tmp_path: Path) -> None:
        mod = _load_check_module(AI_CODE_REVIEW_PATH, "ai_code_review_check")
