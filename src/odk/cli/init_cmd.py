"""odk init — initialize .odk/ in a project."""
...
from odk.core.config import init_config
...
_GUARD_SCRIPT = '''\
#!/usr/bin/env python3
"""ODK unified guard hook — all guard checks in one script."""
...
if tool_name == "Bash" and ("gh pr create" in command or "gh pr merge" in command):
    sys.stderr.write("BLOCKED: Use \'odk task done\' to create PRs — it captures verification proof.\\n")
    sys.exit(2)
# git push is allowed — odk task done needs it internally
# Only gh pr create is blocked (forces proof-based PRs)

# --- GUARD: no-proof-tamper ---
if tool_name in ("Edit", "Write") and ".odk/proofs/" in file_path and "summary.md" not in file_path:
    sys.stderr.write("BLOCKED: Proof files are generated by \'odk task done\'. Do not edit manually.\\n")
    sys.exit(2)
...
# --- GUARD: stage-gate ---
state_file = root / ".odk" / "state.json"
if state_file.exists() and file_path:
    try:
        ...
        if stage in ("01", "02") and file_path.startswith(("src/", "app/", "tests/")):
            if not file_path.startswith(".odk/"):
...
def _install_hooks() -> None:
    """Create .odk/hooks/ with pre-commit and pre-push scripts, set git config."""
    hooks_dir = Path(".odk/hooks")
...
def init_command(
    ...
    try:
        ...
        typer.echo("Error: .odk/config.yaml already exists. Use --force to overwrite.", err=True)
    ...
    console.print(f"[green]ODK initialized for project '{config.project.name}'[/green]\n")
    ...
    schemas_dir = Path(config.components.schemas_path)
    packs_dir = Path(".odk") / "ignition-packs"
    ...
    if search_catalog:
        ...
        if items:
            ...
            console.print("\nRun [bold]odk catalog install <name>[/bold] to install one.")
        else:
