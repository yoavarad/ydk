"""odk memory — knowledge management and memory."""
...
def _load_memory_config() -> OdkConfig:
    """Load config and return the full OdkConfig."""
    from odk.core.config import load_config

    return load_config()


def _get_engine(cfg: OdkConfig) -> object:  # type: ignore[return-type]  # MemoryEngine is optional dep
    """Lazily import and construct MemoryEngine. Fails gracefully if chromadb missing."""
    try:
        from odk.core.memory import MemoryEngine  # type: ignore[import-untyped]
    except ImportError:
        console.print("[red]chromadb is not installed.[/red] Install with: uv pip install 'odk[memory]'")
...
def _get_extractor(cfg: OdkConfig | None = None) -> object:  # type: ignore[return-type]  # MemoryExtractor is optional dep
    """Lazily import MemoryExtractor. Fails gracefully if strands missing."""
    try:
        from odk.core.extractor import MemoryExtractor  # type: ignore[import-untyped]
    except ImportError:
        console.print("[red]strands-agents is not installed.[/red] Install with: uv pip install 'odk[ai]'")
        raise typer.Exit(code=1) from None
...
def extract(
    ...
    if jsonl_path is None:
        ...
        candidate = Path(f".odk/sessions/{task_id}.jsonl")
...
def retrospective(
    ...
    if proposals:
        ...
        console.print("    Run `odk memory search <topic>` to find specific learnings.")
        console.print("    Review extracted memories in .odk/memory/ for aggregate insights.")
...
def audit(
    ...
    try:
        ...
        if report.groups:
            ...
            console.print("  Run `odk memory consolidate` to merge them.")
    except Exception:
