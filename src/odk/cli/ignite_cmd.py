...
def ignite_command(
    ...
    """Generate project skeleton from installed ignition pack and ODK components."""
    ...
    schemas_dir = project_root / ".odk" / "schemas"
    if not schemas_dir.is_dir() or not list(schemas_dir.glob("*.yaml")):
        console.print("[red]Schemas not installed. Run: odk component init-schemas[/red]")
        raise typer.Exit(1)

    # Check pack installed (supports both new and legacy layout)
    packs_dir = project_root / ".odk" / "ignition-packs"
    legacy_pack_dir = project_root / ".odk" / "ignition-pack"
    has_pack = (packs_dir.is_dir() and list(packs_dir.iterdir())) or legacy_pack_dir.is_dir()
    if not has_pack:
        console.print("[red]No ignition pack installed. Run: odk catalog install <pack-name>[/red]")
        raise typer.Exit(1)
    ...
    if not dry_run:
        if skip_verify:
            ...
            spec_results = project_root / ".odk" / "spec-check-results.json"
            if not spec_results.exists():
                console.print(
                    "[red]Error: Spec verification required. Run: odk spec verify --all-files\n"
                    "Or bypass with: odk ignite --skip-verify (for inherited/brownfield specs)[/red]"
                )
                raise typer.Exit(1)
