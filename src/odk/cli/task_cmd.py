"""odk task — task management and validation."""
...
if TYPE_CHECKING:
    from odk.core.complexity_scorer import LLMProvider
    ...
    from odk.models.pm import Dependency
    from odk.repositories.protocols import LifecycleTaskRepository
...
def _validate_component_refs_cli(refs: list[str], project_root: Path) -> None:
    ...
    from odk.core.task_validator import validate_component_ref
...
def _validate_spec_refs_cli(refs: list[str], project_root: Path) -> None:
    ...
    from odk.core.task_validator import validate_spec_ref
...
def _build_lifecycle() -> TaskLifecycle:
    ...
    from odk.core.task_lifecycle import TaskLifecycle as _TaskLifecycle
...
def create(
    ...
    supersedes, related.
    """
    from odk.models.pm import Dependency as _Dep
...
def _validate_batch_yaml(data: dict[str, object], project_root: Path | None = None) -> list[str]:
    ...
    if project_root is not None:
        from odk.core.config import load_config as _load_cfg
...
def _ensure_labels(repo: object) -> None:
    ...
    try:
        from odk.repositories.github._helpers import run_gh
...
def create_batch(
    ...
    from odk.models.pm import EpicCreate, StoryCreate, TaskCreate
    ...
    from odk.repositories.factory import get_epic_repository, get_story_repository
    ...
    # --- Write batch mapping file (.odk/batch-mapping.json) ---
    import json as _json_batch

    mapping_dir = Path(".odk")
...
def start(
    ...
    if not (Path(".odk") / "todos.yaml").exists():
        console.print("[red]No TODO registry found. Run Stage 01.5 (odk ignite) first.[/red]")
        raise typer.Exit(1)
    ...
    try:
        ...
        if cfg.memory.auto_bootstrap:
            from odk.cli.memory_cmd import _get_engine
...
def done(
    ...
    try:
        from odk.core.state import ProjectState as _PS
    ...
    try:
        ...
        if cfg.memory.auto_extract:
            from odk.cli.memory_cmd import _get_engine, _get_extractor
            ...
            if jsonl_path is None:
                jsonl_path = Path(f".odk/sessions/{task_id}.jsonl")
...
def add_gate(
    ...
    from odk.models.gate import Gate as _Gate
    from odk.models.gate import GateType
...
def validate_dag_cmd(
    ...
    try:
        from odk.core.task_validator import check_hierarchy
        from odk.models.pm import EpicSummary as _EpicSummary
        from odk.models.pm import StorySummary as _StorySummary
        from odk.models.pm import TaskSummary as _TaskSummary
...
def component_coverage_cmd(
    exclude: list[str] = typer.Option([], "--exclude", help="Glob patterns to exclude (e.g. 'odk:page:*')"),  # noqa: B008
    ...
    root = Path(".")
    components_dir = root / ".odk" / "components"
    if not components_dir.is_dir():
        typer.echo("No .odk/components/ directory found.")
        return
...
def coverage(
    ...
    if spec_dir is None:
        try:
            from odk.core.config import load_config as _load_cfg_cov
    ...
    if from_file is not None:
        ...
        try:
            from odk.core.config import load_config as _load_config
    ...
    try:
        ...
        for s in stories:
            ...
            try:
                from odk.repositories.github.parser import _parse_body
...
def create_epic(
    ...
    from odk.models.pm import EpicCreate
...
def create_story(
    ...
    from odk.models.pm import StoryCreate
    from odk.repositories.factory import get_story_repository
...
def quick(
    ...
    from odk.core.quickdev import QuickDevSetup
        ...
        tasks.append(
            {
                ...
                "story": story_id,
                "component_refs": [f"odk:contract:{svc_name}/{pascal}Service"],
