"""odk spec -- spec management and quality checks."""
...
from odk.models.evaluation import (
    ComponentFinding,
    CriterionResult,
...
def _run_reviewer_agents(
    ...
    assert isinstance(config, OdkConfig)
    ...
    # Also load custom reviewers from .odk/spec-reviewers/custom/ if present
...
def _run_cached_fanout(
    ...
    *,
    ...
    from odk.core.reviewer_engine import ReviewerEngine
    from odk.models.config import OdkConfig
...
def _format_report_human(
    ...
    lines.append("[bold] ODK Spec Verification Report[/bold]")
    lines.append("[bold]" + "=" * 47 + "[/bold]")
    lines.append("")
    ...
    if report.scanner_result.unlinked_mentions:
        ...
        lines.append(
            ...
            "concepts mentioned without [odk:...] references:"
        )
        lines.extend(
...
def _format_structured_report(
    ...
    # -- Header --
    console.print(f"\n{separator}")
    console.print(" [bold]ODK Spec Verification Report[/bold]")
    console.print(
...
def _dump_report_files(
    ...
    *,
    ...
    """Write plain-text and JSON report files to .odk/reports/.
    ...
    file_console.print(" ODK Spec Verification Report")
    file_console.print(
        f" Project: {project_name} | Files: {file_count}"
        f" | Components: {component_count} | Duration: {duration_seconds:.0f}s"
    )
    ...
    try:
        from odk.core.config import load_config as _lc
        ...
        default_results = Path(".odk/spec-check-results.json")
...
def verify(
    ...
    if not _boto3_available() and not _strands_available():
        console.print(
            "[red]Error:[/red] Spec reviewers require boto3 (a core ODK dependency).\n"
            "Your installation appears broken. Reinstall with: uv pip install odk"
        )
    ...
    if spec_files:
        ...
        if not llm_available and verbose:
            typer.echo(
                ...
                "Only deterministic checks ran. Install: uv pip install 'odk[ai]'"
            )
