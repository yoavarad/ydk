...
Input: test-plan.yaml (ODK_COMPONENTS_TEST_PLAN), user-stories.yaml (ODK_COMPONENTS_USER_STORY)
...
def _render_e2e_test(story: dict) -> str:
    ...
    lines.append(f"        # TODO[odk:test:e2e:{story_id}@test-plan.yaml]")
    if steps:
        lines.append("        # Steps:")
...
def _render_e2e_stub_from_story(story: dict) -> str:
    ...
    lines = [
        ...
        f"        # TODO[odk:test:e2e:{story_id}@user-stories.yaml]",
...
def main() -> None:
    ...
    user_stories_path = os.environ.get("ODK_COMPONENTS_USER_STORY", "")
