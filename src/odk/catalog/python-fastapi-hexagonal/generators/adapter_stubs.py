...
1. ODK_COMPONENTS_ADAPTER — explicit adapter definitions (legacy database adapters)
2. ODK_COMPONENTS_CONTRACT + ODK_COMPONENTS_EXT — generates external service adapters
...
def _ext_id_to_tech_name(ext_id: str) -> str:
    ...
    "odk:ext:market-data/yfinance" -> "YFinance"
    "odk:ext:broker/alpaca" -> "Alpaca"
    "odk:ext:scheduler/apscheduler" -> "APScheduler"
    "odk:ext:auth/cognito" -> "Cognito"
    """
...
def _ext_id_to_domain(ext_id: str) -> str:
    ...
    "odk:ext:market-data/yfinance" -> "market-data"
    "odk:ext:broker/alpaca" -> "broker"
    """
    # Strip odk:ext: prefix
    stripped = ext_id
    if stripped.startswith("odk:ext:"):
        stripped = stripped[len("odk:ext:") :]
...
def main() -> None:
    ...
    ports_path = os.environ.get("ODK_COMPONENTS_CONTRACT", "")
    ext_path = os.environ.get("ODK_COMPONENTS_EXT", "")
