...
def _derive_error_class(raw: str) -> str:
    """Convert an ODK error ID to a PascalCase error class name.

    "odk:error:strategy/strategy-not-found" -> "StrategyNotFoundError"
    "odk:error:broker/AlpacaAdapterError" -> "AlpacaAdapterError"
    "odk:error:auth/token-expired-error" -> "TokenExpiredError"
    "StrategyNotFoundError" -> "StrategyNotFoundError" (passthrough)
    """
    if ":" in raw or "/" in raw:
        # ODK ID format: take last segment after /
        if "/" in raw:
            slug = raw.rsplit("/", 1)[1]
        else:
            slug = raw.rsplit(":", 1)[1]
...
def build_service_context(uc: dict) -> dict:
    ...
    # Normalize ODK map-format methods to list format
    ...
    for m in methods_raw:
        for e in m.get("errors", []):
            ...
            # Derive class name from ODK error ID: "odk:error:strategy/strategy-not-found" -> "StrategyNotFoundError"
...
def _infer_domain_services(
    ...
    for tag, endpoints in groups.items():
                ...
                if not after_id:
                    methods.setdefault(
                        "get",
                        {
                            ...
                            "raises": [f"odk:error:{singular}/{singular}-not-found"],
                        },
                    )
                else:
        ...
        synthetic = {
            "id": f"odk:contract:{tag}/{svc_name}",
...
def main() -> None:
    ...
    route_path = os.environ.get("ODK_COMPONENTS_ROUTE", "")
