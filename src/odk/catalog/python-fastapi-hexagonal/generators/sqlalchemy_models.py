...
Generates SQLAlchemy 2.0 Mapped ORM model files from ODK entity components.
Input: ODK_COMPONENTS_ENTITY
...
def _derive_fk_table(entity_ref: str) -> str:
    ...
    "Strategy" -> "strategies"
    "odk:entity:strategies/Strategy" -> "strategies"
    "StrategyPortfolio" -> "strategy_portfolios"
    """
    # Strip ODK ID prefix if present
    name = entity_ref
    if "/" in name:
...
def build_col_args(field: dict) -> str:
    ...
    # Handle Decimal precision from ODK schema
    ...
    if field.get("default") is not None and field.get("default") != "null":
        ...
        # ODK auto: true means auto-generate (timestamps)
...
def build_relationship_context(rel: dict, owner_name: str = "") -> dict:
    ...
    """
    # ODK relationships use 'entity' ref; extract the entity name from ODK ID or plain name
    target = rel.get("target", "") or rel.get("entity", "")
    # Strip ODK entity ID prefix if present: "odk:entity:strategy/Strategy" -> "Strategy"
    if "/" in target:
...
def _build_table_args(entity: dict) -> list[str]:
    ...
    for idx in indexes:
        if not isinstance(idx, dict):
            continue
        # ODK format: {fields: [...], unique: bool, description: ...}
