"""ODK adapter shim — bridges ODK generator conventions to the ODK ignition protocol.

ODK generators:
  - Read artifacts from ODK_COMPONENTS_* env vars
  - Write files directly to disk via Path.write_text()

ODK ignition generators:
  - Read component data from ODK_COMPONENTS_* env vars (YAML file paths)
  - Read init answers from ODK_INIT_ANSWERS env var (JSON dict)
  - Print JSON array of [{"path": "...", "content": "..."}] to stdout

This adapter:
  1. Reads ODK_COMPONENTS_CONTRACT to build a cli-commands.yaml equivalent dict
  2. Provides a collect() context manager that intercepts file writes and
     returns them as ODK GeneratedFile-compatible dicts
"""
...
def load_commands_from_odk_components() -> dict:
    """Assemble a cli-commands.yaml-equivalent dict from ODK component data.

    ODK passes assembled components via ODK_COMPONENTS_<TYPE> env vars,
    ...
    a direct cli-commands file path via ODK_CLI_COMMANDS for standalone use.
    """
    # Try contract component first (primary ODK path)
    ...
    commands_path = os.environ.get("ODK_CLI_COMMANDS", "")
...
def load_init_answers() -> dict[str, str]:
    """Load init answers from ODK_INIT_ANSWERS env var."""
    raw = os.environ.get("ODK_INIT_ANSWERS", "{}")
    return json.loads(raw)


def emit(files: list[dict[str, str]]) -> None:
    """Print the ODK-protocol JSON array to stdout."""
