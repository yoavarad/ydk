#!/usr/bin/env python3
"""Generator: resolve-commands (ODK ignition pack).

Resolve cli-intent + openapi.json into full cli-commands.yaml.

When both cli-intent and openapi.json are present in ODK components,
...
ODK protocol: reads ODK_COMPONENTS_* env vars, prints JSON array to stdout.
"""
...
def main() -> None:
    ...
    # In ODK, openapi and intent data come through components
    openapi_path = os.environ.get("ODK_COMPONENTS_OPENAPI", "")
    intent_path = os.environ.get("ODK_COMPONENTS_INTENT", "")
    ...
    print(json.dumps([{"path": ".odk/resolved/cli-commands.yaml", "content": content}]))
