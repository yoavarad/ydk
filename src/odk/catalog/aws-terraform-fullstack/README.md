...
ODK ignition pack that generates production-ready Terraform infrastructure for
...
This pack consumes standard ODK component types. Both are optional -- the pack
can generate infrastructure from its own `infrastructure.yaml` fixture alone.

| Component Type | Schema Ref                  | Version   | Required |
|----------------|-----------------------------|-----------|----------|
| entity         | odk-core-schemas/entity     | >= 1.0.0  | No       |
| crosscut       | odk-core-schemas/crosscut   | >= 1.0.0  | No       |
...
- `on_demand` -- when explicitly requested via `odk verify`
