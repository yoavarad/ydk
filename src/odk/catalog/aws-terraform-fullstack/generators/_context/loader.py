...
ODK adapter shim: translates ODK_* env vars to the loader interface expected
by infrastructure generators, and emits JSON to stdout for the ODK ignition
engine.
...
def _flush_json() -> None:
    """Emit buffered files as JSON array to stdout (ODK protocol)."""
    if _generated_files:
...
# ---------------------------------------------------------------------------
# Artifact loading — reads from ODK component YAML
# ---------------------------------------------------------------------------
def load_artifact(artifact_name: str = "infrastructure") -> dict:
    """Load artifact data from ODK components.

    Resolution order:
      1. ODK_COMPONENTS_INFRASTRUCTURE env var (ODK ignition engine path)
    """
    # ODK path: components are passed as YAML list; infrastructure pack
    # expects a single dict, so we merge the list into one dict.
    odk_key = f"ODK_COMPONENTS_{artifact_name.upper().replace('-', '_')}"
    odk_path = os.environ.get(odk_key)
    if odk_path and Path(odk_path).exists():
        raw = yaml.safe_load(Path(odk_path).read_text())
    ...
    print(f"ERROR: Component data not found: {odk_key}", file=sys.stderr)
    sys.exit(1)
...
def get_output_dir() -> Path:
    """Get the output directory from ODK_OUTPUT_DIR."""
...
def write_output(filename: str, content: str) -> None:
    """Buffer generated content for JSON output (ODK protocol)."""
