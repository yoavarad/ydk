...
(ODK-TODO-NNN) and is tracked through open -> in-progress -> done.
"""
...
from odk.models.todo import TodoItem, TodoRegistry, TodoStatus

logger = logging.getLogger("odk.todo_manager")

_TODO_COMMENT_RE = re.compile(r"#\s*ODK-TODO-(\d+):\s*(.*)")
...
class TodoManager:
    """Core manager for ODK TODO tracking."""

    def __init__(self, project_root: Path) -> None:
        self._root = project_root
        self._registry_path = project_root / ".odk" / "todos.yaml"
    ...
    def register(
        ...
        registry = self._load()
        todo_id = f"ODK-TODO-{registry.next_id:03d}"
    ...
    def verify_done(self, todo_id: str) -> bool:
        ...
        if f"ODK-TODO-{int(todo_id.split('-')[-1]):03d}" in window and "raise NotImplementedError" in window:
            return False
