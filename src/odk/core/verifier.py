...
logger = logging.getLogger("odk.verifier")
...
class Verifier:
    ...
    - project_verifications: project-specific (e.g. .odk/verifications/)
    - global_verifications: ships with ODK (e.g. odk/verifications/)
    ...
    def __init__(
        ...
        *,
        ...
        self._project = project_verifications or (project_root / ".odk" / "verifications")
        self._enabled = enabled_plugins  # None = run all; [] = none; [...] = filter
        self._use_cache = use_cache
        self._cache = VerificationCache(project_root / ".odk" / "cache" / "verification")
    ...
    def save_proof(self, report: VerificationReport, task_id: str | None = None) -> Path:
        """Save proof artifact to .odk/proofs/."""
        proofs_dir = self._root / ".odk" / "proofs"
        if task_id:
