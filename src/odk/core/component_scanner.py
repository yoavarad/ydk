...
from odk.models.component import ScannerFinding, ScannerResult

if TYPE_CHECKING:
    from odk.models.component import SchemaDefinition
...
def _build_scanner_prompt_template() -> str:
    ...
    lines = [
        "You are a specification reference auditor for the ODK component manifest system.",
        "",
        "In this system, every structured concept in a specification MUST be referenced",
        "by its full component ID in square brackets, like [odk:entity:orders/Order]",
        "or [odk:route:orders/create].",
        "",
        "Your job: Read the narrative document below and identify ANY concept that is",
        "mentioned in prose without a proper [odk:...] reference.",
        "",
        ...
        "prose mentions of that kind of concept without a proper [odk:...] reference.",
        "",
        ...
        '- BAD: "the Order model stores..." -> should be [odk:entity:orders/Order]',
        '- BAD: "creates a new User record" -> should be [odk:entity:users/User]',
        "",
        'API endpoints (commonly type "route"):',
        '- BAD: "POST to create an order" -> should be [odk:route:orders/create]',
        '- BAD: "calls the login endpoint" -> should be [odk:route:auth/login]',
        "",
        'Error responses (commonly type "error"):',
        '- BAD: "returns insufficient balance error"',
        "  -> should be [odk:error:orders/insufficient-balance]",
        "",
        'Business rules (commonly type "req"):',
        '- BAD: "the system validates the balance"',
        "  -> should be [odk:req:orders/validate-balance]",
        "",
        'External dependencies (commonly type "ext"):',
        '- BAD: "calls Binance for the price" -> should be [odk:ext:binance/ticker]',
        "",
        ...
        "  -> These field definitions belong in [odk:entity:orders/Order] manifest, not prose",
        '- BAD: "POST /api/v1/orders expects {{ symbol: string, side: enum }}"',
        "  -> This API shape belongs in [odk:route:orders/create] manifest",
        "- BAD: \"Returns 422 with {{ type: 'insufficient_balance' }}\"",
        "  -> This error shape belongs in [odk:error:orders/insufficient-balance] manifest",
        ...
        "  -> NFRs with adjectives belong in [odk:nfr:...] with numeric targets",
        "",
        ...
        "project component.",
        "",
        "Do NOT report text that is already inside [odk:...] brackets.",
        "",
        "Return JSON:",
        "{{",
        '  "findings": [',
...
class ComponentScanner:
    ...
    [odk:...] references by sending content to an LLM with a
