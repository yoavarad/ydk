"""Ignition engine — reads installed packs + ODK components, runs generators, produces skeleton."""
...
logger = logging.getLogger("odk.ignition")
...
class IgnitionEngine:
    """Reads installed ignition pack + ODK components, runs generators, produces skeleton."""

    def __init__(self, project_root: Path) -> None:
        self._root = project_root.resolve()
        self._packs_dir = self._root / ".odk" / "ignition-packs"
        self._pack_dir: Path | None = None  # resolved lazily in _load_pack
        self._components_dir = self._root / ".odk" / "components"
    ...
    def ignite(
        ...
        if not component_data:
            msg = "No components found in .odk/components/. Run Stage 01 brainstorming first."
            raise IgnitionError(msg)
        ...
        state = ScaffoldState(self._root / ".odk" / "scaffold-state.yaml")
    ...
    def _load_pack(self) -> dict | None:
        ...
        Auto-detects the single installed pack subdirectory under .odk/ignition-packs/.
        Also supports legacy .odk/ignition-pack/ layout for backwards compatibility.
        """
        # New layout: .odk/ignition-packs/<pack-name>/manifest.yaml
        ...
        # Legacy layout: .odk/ignition-pack/manifest.yaml
        legacy_dir = self._root / ".odk" / "ignition-pack"
    ...
    def _assemble_components(self) -> dict[str, list[dict]]:
        """Read all .odk/components/, group by type, return as lists."""
            ...
            for art_name, art_ref in phase.get("artifacts", {}).items():
                ...
                if "." in ref_str:
                    ...
                    if source_phase in exports and export_name in exports[source_phase]:
                        artifact_env[f"ODK_ARTIFACT_{art_name.upper()}"] = exports[source_phase][export_name]
                    else:
                        errors.append(
    ...
    def _run_generator(
        ...
        """
        env_vars = {
            "ODK_PROJECT_ROOT": str(output_dir),
            "ODK_OUTPUT_DIR": str(output_dir),
            "ODK_INIT_ANSWERS": json.dumps(init_answers),
        }
        ...
        try:
            for comp_type, components in component_data.items():
                fd, tmp_str = tempfile.mkstemp(suffix=".yaml", prefix=f"odk_{comp_type}_")
                os.close(fd)
                tmp = Path(tmp_str)
                tmp.write_text(yaml.dump(components, default_flow_style=False))
                temp_files.append(tmp)
                env_key = f"ODK_COMPONENTS_{comp_type.upper()}"
                env_vars[env_key] = str(tmp)

                # Singleton unwrapping: if exactly 1 component, also provide singular form
                if len(components) == 1:
                    singular_key = f"ODK_COMPONENT_{comp_type.upper()}"
                    fd2, tmp_str2 = tempfile.mkstemp(suffix=".yaml", prefix=f"odk_{comp_type}_singular_")
                    os.close(fd2)
                    tmp2 = Path(tmp_str2)
