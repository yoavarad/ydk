...
class WorktreeManager:
    ...
    def create(self, task_id: str, description: str = "", base_branch: str | None = None) -> Path:
        ...
        Worktree: .odk/worktrees/{task_id}/
        ...
        if git_indicator.is_file():
            msg = (
                ...
                "Run 'odk task start' from the main repository checkout instead."
            )
        ...
        worktree_path = self._root / ".odk" / "worktrees" / task_id
    ...
    def get_worktree_path(self, task_id: str) -> Path | None:
        ...
        path = self._root / ".odk" / "worktrees" / task_id
        return path if path.exists() else None

    def list_worktrees(self) -> list[str]:
        """List active worktree task IDs."""
        wt_dir = self._root / ".odk" / "worktrees"
