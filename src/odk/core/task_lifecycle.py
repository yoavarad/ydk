...
from odk.models.gate import GateStatus
...
logger = logging.getLogger("odk.task_lifecycle")
...
class TaskLifecycle:
    ...
    def start(
        ...
        active_task_file = self._root / ".odk" / "active-task.json"
    ...
    def done(self, task_id: str, *, summary: str | None = None, skip_plugins: list[str] | None = None) -> dict:
        ...
        if not all_passed:
            from odk.core.auto_repair import RepairLoop
        ...
        if self._task_has_ui_components(task):
            screenshots_dir = self._root / ".odk" / "proofs" / task_id / "screenshots"
            if not screenshots_dir.is_dir() or not list(screenshots_dir.iterdir()):
                msg = (
                    ...
                    f".odk/proofs/{task_id}/screenshots/ before running odk task done."
                )
        ...
        proof_dir = self._root / ".odk" / "proofs" / task_id
    ...
    def _check_xfail_removed(self, task_id: str, task_files: list[str]) -> list[str]:
        ...
        for test_file in test_files:
            ...
            # Find xfail markers that reference ODK TODOs
            xfails = re.findall(r'@pytest\.mark\.xfail\(reason="(ODK-TODO-\d+[^"]*?)"\)', content)
            for xfail_reason in xfails:
                todo_match = re.search(r"ODK-TODO-(\d+)", xfail_reason)
                if todo_match:
                    todo_id = f"ODK-TODO-{todo_match.group(1)}"
    ...
    def _task_has_ui_components(task: object) -> bool:
        ...
        if not isinstance(task, TaskDetail):
            return False
        ui_prefixes = ("odk:page:", "odk:route:")
    ...
    def _write_verified_flag(self) -> None:
        """Write a .odk/.verified flag file with a timestamp.

        The pre-push hook checks this flag: if recent (< 5 minutes), it
        skips verification to avoid double-running after ``odk task done``.
        """
        flag_path = self._root / ".odk" / ".verified"
    ...
    def _collect_screenshots(self, task_id: str) -> list[str]:
        """Check for screenshots in .odk/proofs/<task_id>/screenshots/."""
        screenshots_dir = self._root / ".odk" / "proofs" / task_id / "screenshots"
        ...
        for img in sorted(screenshots_dir.iterdir()):
            if img.suffix.lower() in (".png", ".jpg", ".jpeg", ".gif", ".webp"):
                rel_path = f".odk/proofs/{task_id}/screenshots/{img.name}"
