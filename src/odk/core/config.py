...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "spec_check": {
        ...
        "global_timeout": 120,
        "concurrency": 10,
        "results_path": ".odk/spec-check-results.json",
        "thresholds": {
            "completeness": 8,
            "clarity": 8,
            "architecture": 8,
...
def load_config(config_path: Path = Path(".odk/config.yaml")) -> OdkConfig:
    """Load and validate config. Returns defaults if file doesn't exist."""
    if not config_path.is_file():
        return OdkConfig.model_validate(DEFAULT_CONFIG)

    raw = yaml.safe_load(config_path.read_text())
    return OdkConfig.model_validate(raw)


def save_config(config: dict[str, Any], config_path: Path = Path(".odk/config.yaml")) -> None:
...
def init_config(
    project_name: str,
    config_path: Path = Path(".odk/config.yaml"),
    force: bool = False,
    remote: str | None = None,
    stack: str | None = None,
) -> OdkConfig:
    """Initialize .odk/config.yaml with defaults. Raises if exists and not force."""
    ...
    if stack:
        from odk.core.stacks import get_stack
    ...
    return OdkConfig.model_validate(data)
...
def set_config_value(config: dict[str, Any], key_path: str, value: str) -> dict[str, Any]:
    ...
    try:
        OdkConfig.model_validate(config)
