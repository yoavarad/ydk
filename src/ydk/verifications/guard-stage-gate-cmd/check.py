#!/usr/bin/env python3
"""Guard plugin: block YDK commands inappropriate for the current development stage."""
...
_STAGE_CMD_RULES: dict[str, dict[str, list[str]]] = {
    "01": {
        "blocked": ["ydk ignite", "ydk task create", "ydk task start"],
        "allowed": ["ydk component", "ydk spec", "ydk catalog"],
    },
    "01.5": {
        "blocked": ["ydk task create-batch", "ydk task start"],
        "allowed": ["ydk ignite", "ydk todo list", "ruff", "pytest --collect-only"],
    },
    "02": {
        "blocked": ["ydk ignite"],
        "allowed": ["ydk task", "ydk todo"],
    },
    "03": {
        "blocked": ["ydk ignite"],
        "allowed": ["ydk task", "ydk verify", "pytest", "ruff"],
    },
}
...
def main() -> None:
    ...
    state_path = root / ".ydk" / "state.json"
    if not state_path.exists():
        result = {
            "name": "guard-stage-gate-cmd",
            "passed": True,
            "output": "PASS: no .ydk/state.json — stage gate not active",
