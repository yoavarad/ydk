...
from ydk.models.navigation import ComponentCoverage, NavigationStatus, ProjectStage
...
def detect_stage(ydk_root: Path) -> ProjectStage:
    """Detect which YDK stage the project is in based on artifacts present."""
    if not ydk_root.is_dir():
        return ProjectStage.EMPTY

    specs_dir = ydk_root / "specs"
    tasks_dir = ydk_root / "tasks"
...
def recommend_next_action(stage: ProjectStage, task_counts: dict[str, int]) -> str:
    ...
    if stage == ProjectStage.EMPTY:
        return "Run `ydk init` to initialize the project."
    if stage == ProjectStage.INITIALIZED:
        return "Create specs with `ydk spec create` to define your project."
    if stage == ProjectStage.SPECIFIED:
        return "Specs exist but no tasks. Run `ydk task create` to break specs into tasks."
    if stage == ProjectStage.TASKED:
        open_count = task_counts.get("open", 0)
        if open_count > 0:
            return f"{open_count} task(s) ready. Run `ydk task ready` to pick the next one."
        return "All tasks started. Monitor progress with `ydk status`."
    if stage == ProjectStage.IN_PROGRESS:
        ...
        return f"{in_progress} task(s) in progress. Complete them with `ydk task done <id>`."
...
def scan_project(project_root: Path) -> NavigationStatus:
    ...
    ydk_root = project_root / ".ydk"

    stage = detect_stage(ydk_root)

    specs_dir = ydk_root / "specs"
    adrs_dir = ydk_root / "adrs"
    tasks_dir = ydk_root / "tasks"
    components_dir = ydk_root / "components"
    stories_dir = ydk_root / "stories"
    epics_dir = ydk_root / "epics"
