...
from ydk.models.component import ComponentId, ComponentManifest, SchemaDefinition, SchemaField

if TYPE_CHECKING:
    from pathlib import Path

logger = logging.getLogger("ydk.component_registry")
...
class ComponentRegistry:
    ...
    def resolve_id(self, component_id: str) -> Path:
        """Convert ydk:type:namespace/name to a file path under components_dir."""
    ...
    def validate_component(self, component_id: str) -> list[str]:
        ...
        if not schema_ref.startswith("ydk:schema:"):
            return [f"Invalid $schema format: {schema_ref}"]

        schema_type = schema_ref.removeprefix("ydk:schema:")
