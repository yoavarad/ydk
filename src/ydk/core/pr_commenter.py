...
class PRCommenter:
    ...
    def post_verification_results(self, pr_url: str, report: VerificationReport) -> None:
        ...
        If a previous YDK comment exists (identified by hidden marker),
    ...
    def format_comment(self, report: VerificationReport) -> str:
        ...
        lines.append(f"## {icon} YDK Verification — {status}")
        lines.append("")
    ...
    def _find_existing_comment(self, pr_url: str) -> str | None:
        """Find an existing YDK verification comment on the PR.
