...
from ydk.models.component import LinkerResult
...
_NARRATIVE_REF_PATTERN = re.compile(r"\[ydk:[a-zA-Z0-9_-]+:[a-zA-Z0-9/_.-]+\]")
_CROSS_REF_PATTERN = re.compile(r"ydk:[a-zA-Z0-9_-]+:[a-zA-Z0-9/_.-]+")


class ComponentLinker:
    """Deterministic reference linker (Layer A).

    Scans narrative markdown files for [ydk:...] references and validates
    ...
    def scan_narratives(self) -> list[tuple[Path, list[str]]]:
        """Find all [ydk:...] references in markdown files under narratives_dir."""
        ...
        for yaml_file in self._registry._components_dir.rglob("*.yaml"):
            ...
            for match in _CROSS_REF_PATTERN.finditer(content):
                ...
                if ref_id == component_id:
                    continue
                if ref_id.startswith("ydk:schema:"):
                    continue
    ...
    def _collect_narrative_refs(self) -> set[str]:
        """Collect all ydk:... refs from narrative markdown files."""
        refs: set[str] = set()
    ...
    def _collect_manifest_refs(self, known_ids: set[str]) -> set[str]:
        ...
        for yaml_file in self._registry._components_dir.rglob("*.yaml"):
            ...
            for match in _CROSS_REF_PATTERN.finditer(content):
                ref_id = match.group()
                if ref_id != component_id and not ref_id.startswith("ydk:schema:"):
                    refs.add(ref_id)
        return refs
