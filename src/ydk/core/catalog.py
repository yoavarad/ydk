...
from ydk.models.catalog import CatalogItem, CatalogManifest

if TYPE_CHECKING:
    from pathlib import Path

logger = logging.getLogger("ydk.catalog")
...
class CatalogBackend(Protocol):
    ...
    def install(self, name: str, version: str | None, project_root: Path) -> None:
        """Install a catalog item into a project's .ydk/ directories."""
        ...
...
class LocalCatalogBackend:
    """Reads from ~/.ydk/catalog/, the package built-in catalog, and project .ydk/catalog/."""

    def __init__(self, catalog_dir: Path | None = None, *, include_builtin: bool = True) -> None:
        from pathlib import Path as _Path

        self._catalog_dir = catalog_dir or _Path.home() / ".ydk" / "catalog"
        # Built-in catalog shipped with the ydk package
    ...
    def _iter_items(self) -> list[tuple[Path, CatalogManifest]]:
        ...
        Scans both the package built-in catalog and the user catalog (~/.ydk/catalog/).
    ...
    def install(self, name: str, version: str | None, project_root: Path) -> None:
        """Install a catalog item into a project's .ydk/ directories based on tags."""
        item = self.get(name, version)
        if item is None:
            msg = f"Catalog item '{name}' not found"
            raise ValueError(msg)

        ydk_dir = project_root / ".ydk"
        ...
        for tag in item.tags:
            if tag in _TAG_INSTALL_DIRS:
                dest = ydk_dir / _TAG_INSTALL_DIRS[tag] / item.name
        ...
        if not installed:
            # Default: copy to .ydk/catalog-installed/<name>
            dest = ydk_dir / "catalog-installed" / item.name
            dest.parent.mkdir(parents=True, exist_ok=True)
            if dest.exists():
                shutil.rmtree(dest)
            shutil.copytree(item.path, dest)

        # Record in .ydk/catalog-lock.yaml
        self._record_installed(project_root, item)

    def _record_installed(self, project_root: Path, item: CatalogItem) -> None:
        """Record an installed item in .ydk/catalog-lock.yaml."""
        lock_path = project_root / ".ydk" / "catalog-lock.yaml"
        ...
        for entry in installed_list:
            ...
            results.append(
                CatalogItem(
                    ...
                    tags=entry.get("tags", []),
                    path=project_root / ".ydk",  # approximate
                )
            )
        return results
        ...
        if isinstance(tags, list):
            for tag in tags:
                if isinstance(tag, str) and tag in _TAG_INSTALL_DIRS:
                    dest = ydk_dir / _TAG_INSTALL_DIRS[tag] / name
                    if dest.exists():
                        shutil.rmtree(dest)

        default_dest = ydk_dir / "catalog-installed" / name
