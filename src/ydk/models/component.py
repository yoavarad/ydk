...
_ID_RE = r"^ydk:[a-z][a-z0-9_-]*:[a-zA-Z0-9_-]+(/[a-zA-Z0-9_.-]+)+$"
_ID_RE_NO_SLASH = r"^ydk:[a-z][a-z0-9_-]*:[a-zA-Z0-9_.-]+$"
...
class ComponentId(BaseModel):
    ...
    def validate_id_format(cls, v: str) -> str:
        """Ensure ID matches ydk:<type>:<namespace>/<name> pattern."""
        if not v.startswith("ydk:"):
            msg = f"Component ID must start with 'ydk:': {v}"
    ...
    def parse(cls, full_id: str) -> ComponentId:
        ...
        if not full_id.startswith("ydk:"):
            msg = f"Component ID must start with 'ydk:': {full_id}"
            raise ValueError(msg)
...
class ComponentManifest(BaseModel):
    ...
    def validate_schema_ref(cls, v: str) -> str:
        """Ensure $schema follows ydk:schema:<type> format."""
        if not v.startswith("ydk:schema:"):
            msg = f"$schema must start with 'ydk:schema:': {v}"
            raise ValueError(msg)
        return v
    ...
    def validate_id(cls, v: str) -> str:
        """Ensure id follows ydk:<type>:<namespace>/<name> format."""
        if not v.startswith("ydk:"):
            msg = f"Component ID must start with 'ydk:': {v}"
            raise ValueError(msg)
        return v
