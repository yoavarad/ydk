"""ydk config — configuration management commands."""
...
from ydk.core.config import get_config_value, load_config, save_config, set_config_value
from ydk.output.console import console
from ydk.output.formatters import OutputFormat, get_formatter

config_app = typer.Typer(name="config", help="Manage YDK configuration")

CONFIG_PATH = Path(".ydk/config.yaml")
...
def get(
    ...
    if not CONFIG_PATH.is_file():
        typer.echo("Error: no config found. Run 'ydk init' first.", err=True)
