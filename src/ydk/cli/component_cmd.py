...
def _make_registry() -> ComponentRegistry:
    ...
    schemas_dir = Path(".ydk") / "schemas"
    components_dir = Path(".ydk") / "components"
...
def show_component(
    ctx: typer.Context,
    component_id: str = typer.Argument(..., help="Full component ID (ydk:type:namespace/name)"),
) -> None:
...
def create_component(
    ...
    component_id = f"ydk:{type_name}:{namespace_name}"
...
def list_schemas(ctx: typer.Context) -> None:
    ...
    if not schemas:
        console.print("[yellow]No schemas found. Run 'ydk component init-schemas' first.[/yellow]")
        return
