"""ydk catalog — browse, install, publish, and manage catalog items."""
...
from ydk.cli._helpers import format_or_echo, get_output_format
...
from ydk.output.formatters import OutputFormat, get_formatter

catalog_app = typer.Typer(name="catalog", help="Browse and manage the YDK catalog")


def _default_catalog_dir() -> Path:
    return Path.home() / ".ydk" / "catalog"
...
def install(
    ...
    if not format_or_echo(ctx, {"installed": name, "version": version}):
        console.print(f"[green]Installed '{name}' into {project / '.ydk'}[/green]")
...
def info(
    ...
    from ydk.core.catalog import _load_manifest
