(function () {
  ...
  toolbar.id = "ydk-toolbar";
  toolbar.innerHTML = `
    <button data-mode="select" class="ydk-tb-btn">Select Mode</button>
    <button data-mode="comment" class="ydk-tb-btn">Comment Mode <kbd>C</kbd></button>
    <button data-mode="draw" class="ydk-tb-btn">Draw Mode <kbd>R</kbd></button>
    <span class="ydk-tb-sep">|</span>
    <button data-mode="none" class="ydk-tb-btn ydk-tb-cancel">Cancel <kbd>Esc</kbd></button>
  `;
  document.body.prepend(toolbar);

  function setMode(mode) {
    currentMode = mode;
    document.querySelectorAll(".ydk-tb-btn").forEach(function (btn) {
      btn.classList.toggle("ydk-tb-active", btn.dataset.mode === mode);
    });
  ...
  function clearHighlight() {
    if (highlightedEl) {
      highlightedEl.classList.remove("ydk-highlight");
      highlightedEl = null;
    }
  }
  ...
  document.addEventListener("mousemove", function (e) {
    ...
    if (el.closest("#ydk-toolbar") || el.closest(".ydk-popover")) return;
    clearHighlight();
    el.classList.add("ydk-highlight");
    highlightedEl = el;
  });
  ...
  function showPopover(anchorEl, rect, onSubmit) {
    ...
    pop.innerHTML = `
      <textarea class="ydk-popover-text" rows="3" placeholder="Add a comment..."></textarea>
      <div class="ydk-popover-actions">
        <button class="ydk-popover-submit">Submit</button>
        <button class="ydk-popover-cancel">Cancel</button>
      </div>
    `;
    document.body.appendChild(pop);
    var ta = pop.querySelector("textarea");
    ta.focus();
    pop.querySelector(".ydk-popover-submit").addEventListener("click", function () {
      var text = ta.value.trim();
      if (text) onSubmit(text);
      removePopover();
    });
    pop.querySelector(".ydk-popover-cancel").addEventListener("click", removePopover);
  }

  function removePopover() {
    var existing = document.querySelector(".ydk-popover");
  ...
  document.addEventListener("click", function (e) {
    if (currentMode === MODE.COMMENT) {
      ...
      if (choice) {
        ...
        document.querySelectorAll("[data-choice]").forEach(function (c) { c.classList.remove("ydk-selected"); });
        choice.classList.add("ydk-selected");
      }
    }
  }, true);
  ...
  document.addEventListener("mousedown", function (e) {
    ...
    if (e.target.closest("#ydk-toolbar") || e.target.closest(".ydk-popover")) return;

    var overlay = document.createElement("div");
    overlay.className = "ydk-draw-overlay";
    document.body.appendChild(overlay);

    var rect = document.createElement("div");
    rect.className = "ydk-draw-rect";
