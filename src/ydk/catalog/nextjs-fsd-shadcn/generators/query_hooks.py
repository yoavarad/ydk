...
Input: YDK_ARTIFACT_OPENAPI (preferred) | YDK_COMPONENTS_ROUTE (fallback)
...
def _endpoints_from_routes(routes: list[dict]) -> list[dict]:
    """Convert YDK route components to endpoint dicts."""
    endpoints = []
    for route in routes:
...
def main() -> None:
    ...
    if openapi_path and Path(openapi_path).exists():
        ...
        routes_path = os.environ.get("YDK_COMPONENTS_ROUTE", "")
        if not routes_path or not Path(routes_path).exists():
            print(
                "Warning: Neither YDK_ARTIFACT_OPENAPI nor YDK_COMPONENTS_ROUTE available — query-hooks deferred.",
                file=sys.stderr,
            )
            print(json.dumps([]))
            return
