...
Output: src/shared/api/generated/ (written by bunx, then read back into YDK protocol)
...
def _collect_output(output_dir: Path) -> list[dict]:
    """Read generated .ts files and return as YDK generator protocol [{path, content}]."""
...
def main() -> None:
    project_root_str = os.environ.get("YDK_PROJECT_ROOT", "")
    if not project_root_str:
        print("Error: YDK_PROJECT_ROOT is not set", file=sys.stderr)
        sys.exit(1)

    project_root = Path(project_root_str)
    output_dir = Path(os.environ.get("YDK_OUTPUT_DIR", str(project_root / "src/shared/api/generated")))
    output_dir.mkdir(parents=True, exist_ok=True)

    openapi_input = _resolve_openapi_input(dict(os.environ), project_root)
    if openapi_input is None:
        print(
            "Warning: no openapi.json found and YDK_ARTIFACT_OPENAPI not set — SDK generation deferred.",
            file=sys.stderr,
        )
        print(json.dumps([]))
        return
