...
Falls back to basic stubs (based on response status codes from YDK route components)
when test-plan.yaml is absent.

Input: test-plan.yaml (YDK_COMPONENTS_TEST_PLAN), YDK route components (YDK_COMPONENTS_ROUTE)
...
def _render_test_with_scenario(
    ...
    lines.append(f"        # TODO[ydk:test:integration:{endpoint_key}@test-plan.yaml]")
...
def _render_test_stub_from_contract(
    ...
    lines = [
        ...
        f"        # TODO[ydk:test:integration:{method} {path}@YDK route components]",
...
def build_domain_file(
    ...
    if not body_lines:
        ...
        body_lines.append(f"        # TODO[ydk:test:integration:{domain}@YDK route components]")
