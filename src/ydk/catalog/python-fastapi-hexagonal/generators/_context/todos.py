"""TODO marker generation for YDK generator stubs."""
...
def service_todo(
    ...
    Accepts YDK contract format: method_name + method_def dict.
    ...
    lines = [
        f"# YDK-TODO: {service_name}.{method_name}",
        *desc_lines,
...
def adapter_todo(
    ...
    if method_name == "__init__":
        if technology.lower() in ("apscheduler", "apscheduler-async"):
            ...
            return lines
        lines = [
            f"# YDK-TODO: {adapter_name}.__init__",
    ...
    lines = [
        f"# YDK-TODO: {adapter_name}.{method_name}",
...
def test_todo(
    ...
    if method_name:
        ...
        if method_def:
            ...
            output = "?"
        return [
            f"# YDK-TODO: test {subject}.{method_name}",
            f"# IMPLEMENT: Write test assertions for {desc}",
            f"#   Input: {input_summary}",
            f"#   Output: {output}",
        ]
    return [
        f"# YDK-TODO: test {test_type}:{subject}",
        f"# IMPLEMENT: {desc}",
    ]
