...
Input: YDK route components, services.yaml
...
def _build_entity_domain_map() -> dict[str, str]:
    ...
    """
    ac_path = os.environ.get("YDK_COMPONENTS_ROUTE", "")
...
def _load_use_cases() -> dict:
    """Load YDK contract components and index methods by ServiceName.method_name."""
    ...
    for uc in contracts:
        ...
        if isinstance(methods, dict):
            # YDK map format: {method_name: {params: ..., returns: ...}}
            for method_name, method_def in methods.items():
                ...
                # Map YDK 'params' to legacy 'input' format
...
def build_router_context(tag: str, endpoints: list) -> dict:
    ...
    for ep in endpoints:
        ...
        # Get path param types from request.path_params (new YDK format)
